...
// 修改 week05-2 好玩的程式設計 兩層for(迴圈) 配合 if .. else if .. else
void setup() { 
...
void draw(){
  for (int i=0; i<10; i++) { // 上週是for y 現在改成 for i 左手 i
    for (int j=0; j<16; j++) { // 上週是for x 現在改成 for j 右手 j
      if ( a[i][j]==1 ) fill(#5DFF1F); // 若陳列有值, 設綠色
